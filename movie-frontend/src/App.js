...
const MovieApp = () => {
    ...
    const LoginForm = () => {
        ...
        return ( <
            ...
            <
            h1 className = "text-2xl font-bold text-gray-900" > { isLogin ? 'Iniciar Sesión' : 'Crear Cuenta' } <
            /h1> <
            /div>
            ...
            <
            p className = "text-center mt-4 text-sm text-gray-600" > { isLogin ? '¿No tienes cuenta?' : '¿Ya tienes cuenta?' } <
            button type = "button"
            onClick = {
                () => setIsLogin(!isLogin) }
    ...
    const MovieCard = ({ movie, showInteractions = false }) => ( <
            ...
            movie.genres && movie.genres.length > 0 && ( <
                ...
                div className = "flex flex-wrap gap-1" > {
                    movie.genres.slice(0, 3).map((genre, i) => ( <
                        span key = { i }
                        className = "text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full" > { genre } <
                        /span>
                    ))
                } <
                /div> <
...

        {
            showInteractions && ( <
                ...
                <
                button onClick = {
                    () => handleInteraction(movie.id, 'like') }
                ...
                /button> <
                button onClick = {
                    () => handleInteraction(movie.id, 'dislike') }
    ...
    return ( <
        ...
        <
        Navigation / > { activeTab === 'home' && < SearchBar / > }
            ...
            activeTab === 'recommendations' && ( <
...

                {
                    recommendations.length > 0 ? ( <
                        ...
                        Califica algunas películas en la sección "Explorar"
